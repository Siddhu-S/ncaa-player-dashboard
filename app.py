...
def server(input, output, session):
    ...
    def d1_plot_meta():
        ...
        if len(filtered) == 0:
            ...
            def apply_step(label, next_df, detail):
                nonlocal probe, zero_reason
                if zero_reason:
                    return
                probe = next_df
                if len(probe) == 0:
                    zero_reason = f" · zeroed by {label} {detail}"

            qual_mode = input.d1_qualification_filter()
            probe2 = apply_qualification_filter(probe, qual_mode)
            apply_step("qualification", probe2, f"({qual_mode or 'none'})")
            ...
            if not zero_reason and q:
                probe2 = probe[probe["name"].str.lower().str.contains(q, na=False)]
                apply_step("search", probe2, f"({q})")

            def apply_range_probe(label, col, values):
                nonlocal probe
                if zero_reason:
                    return
                lo, hi = values
                probe2 = probe[(probe[col] >= lo) & (probe[col] <= hi)]
                apply_step(label, probe2, f"([{lo}, {hi}])")

            if not zero_reason:
                apply_range_probe("eligibility", "eligibility", input.d1_eligibility())
            if not zero_reason:
                apply_range_probe("mpg", "mpg", input.d1_mpg())
            if not zero_reason:
            ...
            if not zero_reason:
                apply_range_probe("rim_share", "rim_share", input.d1_rim_share())
            if not zero_reason:
                apply_range_probe("mid_share", "mid_share", input.d1_mid_share())
            if not zero_reason:
                apply_range_probe("assisted_fg_pct", "assisted_fg_pct", input.d1_assisted_fg_pct())
            if not zero_reason:
                apply_range_probe("rim_fg_pct", "rim_fg_pct", input.d1_rim_fg_pct())
            if not zero_reason:
                apply_range_probe("mid_fg_pct", "mid_fg_pct", input.d1_mid_fg_pct())
            if not zero_reason:
                apply_range_probe("rim_assisted_pct", "rim_assisted_pct", input.d1_rim_assisted_pct())
            if not zero_reason:
                apply_range_probe("mid_assisted_pct", "mid_assisted_pct", input.d1_mid_assisted_pct())
            if not zero_reason:
                apply_range_probe("three_assisted_pct", "three_assisted_pct", input.d1_three_assisted_pct())
            if not zero_reason:
                apply_range_probe("apg", "apg", input.d1_apg_range())
            if not zero_reason:
                apply_range_probe("bpm", "bpm", input.d1_bpm())
            if not zero_reason:
                apply_range_probe("porpag", "porpag", input.d1_porpag())
            if not zero_reason:
                apply_range_probe("spg", "spg", input.d1_spg_range())
            if not zero_reason:
                apply_range_probe("bpg", "bpg", input.d1_bpg_range())
            if not zero_reason:
                apply_range_probe("ast_tov", "ast_tov", input.d1_ast_tov())
            if not zero_reason:
                apply_range_probe("height", "heightIn", input.d1_height())
